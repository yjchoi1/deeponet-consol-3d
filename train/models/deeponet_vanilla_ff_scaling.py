...
class VanillaDeepONetFF3D(nn.Module):
    ...
    def __init__(
        ...
        # Fourier features (kernel-style RFF for cv + coordinates)
        input_dim = 1 + 4  # Cv plus (t, x, y, z)
        # Dimension-aware scaling: effective sigma scales as 1/sqrt(d)
        ff_sigma_eff = float(ff_sigma) / (input_dim ** 0.5)
        B = torch.randn(input_dim, ff_features, dtype=dtype, device=device) * torch.tensor(ff_sigma_eff, dtype=dtype,
                                                                                           device=device)
    ...
    def forward(self, u: torch.Tensor, cv: torch.Tensor, coord: torch.Tensor) -> torch.Tensor:
        ...
        # Apply Fourier features (no 2π; sqrt(2/m) normalization)
        rff = torch.matmul(trunk_input, self.B)
        phi = torch.cat((torch.sin(rff), torch.cos(rff)), dim=-1)
        phi = phi * (2.0 / float(self.B.shape[1])) ** 0.5
        rff_input = phi
...
def build_vanilla_ff_scaling_model(cfg: Mapping[str, object]) -> VanillaDeepONetFF3D:
